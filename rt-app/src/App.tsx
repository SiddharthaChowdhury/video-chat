...
const App: React.FC<any> = () => {
	...
	const [onlineUsers, setOnline] = React.useState<string[]>([]); 
	let uid: string| null;
	React.useEffect(() => {
		$conn.socket.on(IdSocketKey.onlineUsers, (data: any[]) => {
			console.log('online iDs', data);
			setOnline(data);
		});

		$conn.socket.on(IdSocketKey.userSessionLimit, (sessionCount: number) => {
			alert(`You already logged in from too many devices [count = ${sessionCount}]`)
		});

		uid =  window.prompt('Enter Login ID (assuming unique)');
		const name = window.prompt('Enter name');
		
		$conn.socket.emit(IdSocketKey.login, {id: uid, name });
	}, [])
	...
	return (
		<div>
			...
			<div className={'userlist'}>
				<h4>Online users</h4>
				<ul>
				{onlineUsers.map((usr: any, _key: number) => {
					return (
						<li key={_key}>
							<span>user: {usr}</span>
							{uid !== usr &&
